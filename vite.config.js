...
function cleanUrlsPlugin() {
  ...
  // Dev: project detail pages served from the single [slug].html template.
  // Preview/prod: individual per-slug HTML files exist (generated by build).
  function makeRewrite(useTemplate) {
    return (req, _res, next) => {
      ...
      if (slug) {
        req.url = useTemplate
          ? `/work/[slug].html${slug[2] || ''}`
          : `/work/${slug[1]}.html${slug[2] || ''}`;
        return next();
      }
      // Top-level pages
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(makeRewrite(true));
    },
    configurePreviewServer(server) {
      server.middlewares.use(makeRewrite(false));
    },
  };
}
